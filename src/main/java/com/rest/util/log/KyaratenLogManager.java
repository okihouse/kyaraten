...
public class KyaratenLogManager {

	private static final Logger logger = LoggerFactory.getLogger(KyaratenLogManager.class);
	...
	public static String makeLog(String msg, Exception e) {
		...
		sb.append(msg)
		.append(KyaratenLogConstant.BLOCK_DELEMETER);
	...
	public static String makeLog(Object value, Exception e) {
		StringBuffer sb = new StringBuffer();
		convertObject(sb, value);
		sb.append(KyaratenLogConstant.BLOCK_DELEMETER);
		convertException(sb, e);
		return sb.toString();
	}
	...
	public static String makeLog(int count, Object...value) {
		...
		if (value == null || (count > value.length)) {
			...
			for (int i = count, devideCount = 0; i < value.length; i++, devideCount++) {
				sb.append(value[i]);
				if (uriFlag == true || (value[i] != null && value[i].equals(KyaratenLogConstant.QUESTION_MARK))) {
					uriFlag = false;
					continue;
				}
				if(i == (value.length - 1)) break; // Last
				sb.append(devideCount % 2 == 0 ? KyaratenLogConstant.VALUE_DELEMETER : KyaratenLogConstant.VALUE_DELEMETER_COMMA);
				uriFlag = (value[i] != null && (value[i].equals(KyaratenLogConstant.URI_NAME))) ? true : false;
			}
		}
		return sb.toString();
	}
	...
	private static String convertObject(StringBuffer sb, Object value){
		...
		if (value == null) {
			...
			if (value instanceof List<?>) {
				...
				for (Object obj : list) {
					sb.append(obj)
					  .append(KyaratenLogConstant.NEXT_LINE);
				}
				...
				for (Object obj : set) {
					sb.append(obj)
					  .append(KyaratenLogConstant.VALUE_DELEMETER)
					  .append(map.get(obj))
					  .append(KyaratenLogConstant.NEXT_LINE);
				}
			} else {
	...
	private static String convertException(StringBuffer sb, Exception e){
		...
		.append(KyaratenLogConstant.STRING_DELEMETER)
		.append(e.getMessage());
		
		StackTraceElement[] stackTraceElement = e.getStackTrace();
		for (StackTraceElement element : stackTraceElement) {
			if (element != null && element.toString().startsWith(KyaratenLogConstant.BASE_PACKAGE)) {
				sb.append(KyaratenLogConstant.VALUE_DELEMETER_EXCEPTION)
	...
	private static StringBuffer addCommaDelimiter(StringBuffer sb, boolean isFirst){
		if (isFirst == false) sb.append(KyaratenLogConstant.VALUE_DELEMETER_COMMA);
		return sb;
	}
