...
public class KyaratenLogConstant {
	
	private static final Logger logger = LoggerFactory.getLogger(KyaratenLogConstant.class);
	...
	static{
		try {
			...
			logger.error(KyaratenLogManager.makeLog("Could not obtain IP Address", e));
		}
		
		String fullPackageName = KyaratenLogConstant.class.getCanonicalName();
		String rootPackageName = "";
		try {
			rootPackageName = fullPackageName.substring(0, fullPackageName.indexOf("."));
		} catch (Exception e) {
			logger.error(KyaratenLogManager.makeLog("invalid package naming. package=" + fullPackageName, e));
			rootPackageName = "com";
		}
		BASE_PACKAGE = rootPackageName;
	}
	
	public static final String MODULE_NAME = "KYARATEN";
